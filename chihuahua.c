...
polx *init_sparsecnst_half(sparsecnst *cnst, size_t r, size_t nz, size_t buflen,
  ...
  buf = _malloc(4 * nz * sizeof(size_t) + nz * sizeof(polx *) + 63 +
                buflen * sizeof(polx));
  cnst->idx = (size_t *)buf;
...
void init_sparsecnst_raw(sparsecnst *cnst, size_t r, size_t nz,
...
int set_sparsecnst_raw(sparsecnst *cnst, uint8_t h[16], size_t nz,
                       const size_t idx[nz], const size_t n[nz], size_t deg,
  ...
  if (nz != cnst->nz) {
    fprintf(stderr, "ERROR in set_sparsecnst_raw(): Mismatch in number of "
                    "affected vectors\n");
    return 1;
  }
  if (deg != cnst->deg) {
    fprintf(stderr,
            "ERROR in set_sparsecnst_raw(): Mismatch in extension degree\n");
    return 2;
  }
  ...
  for (j = 0; j < nz; j++) {
    if (idx[j] != cnst->idx[j]) {
      fprintf(stderr,
              "ERROR in set_sparsecnst_raw(): Mismatch in index of %zu-th "
              "affected vector\n",
              j);
      return 4;
    }
...
void sparsecnst_eval(polx *b, const sparsecnst *cnst, polx *sx[],
                     const witness *wt) {
  const size_t r = wt->r;
  const size_t *n = wt->n;
  ...
  for (i = 0; i < cnst->nz; i++) {
    j = cnst->idx[i];
    k = cnst->off[i];
    polxvec_mul_extension(t, cnst->phi[i], &sx[j][k], cnst->len[i],
                          deg2 / cnst->mult[i], cnst->mult[i]);
...
int init_prncplstmnt_raw(prncplstmnt *st, size_t r, const size_t n[r],
  ...
  if (betasq > JLMAXNORMSQ) {
    fprintf(stderr, "ERROR in init_prcplstmnt_raw(): Total witness norm too "
                    "big for JL projection\n");
    return 1;
  }
...
int set_prncplstmnt_lincnst_raw(prncplstmnt *st, size_t i, size_t nz,
  ...
  if (i >= st->k) {
    fprintf(stderr,
            "ERROR in set_prcplstmnt_lincnst_raw(): Constraint %zu does not "
            "exist\n",
            i);
    return 1;
  }
  ...
  if (cnst->idx) {
    fprintf(stderr, "ERROR in set_prncplstmnt_lincnst_raw(): Constraint has "
                    "already been set\n");
    return 2;
  }
  for (j = 0; j < nz; j++) {
    ...
    if (k >= st->r) {
      fprintf(stderr,
              "ERROR in set_prncplstmnt_lincnst_raw(): Witness vector %zu does "
              "not exist\n",
              k);
      return 3;
    }
    if (n[j] * deg != st->n[k]) {
      fprintf(stderr, "ERROR in set_prncplstmnt_lincnst_raw(): Mismatch in "
...
void collaps_sparsecnst(constraint *ocnst, statement *ost, const proof *pi,
                        const sparsecnst *icnst, size_t k) {
  size_t i, j, u, v;
  ...
  __attribute__((aligned(16))) uint8_t hashbuf[16 + j * QBYTES];
  ...
  for (i = 0; i < k; i++) {
    ...
    for (j = 0; j < icnst->nz; j++) {
      ...
      polxvec_scale_add(&(*phi)[v + icnst->off[j]], icnst->phi[j],
                        icnst->len[j], s);
    }
...
void aggregate_sparsecnst(statement *ost, const proof *pi,
                          const sparsecnst *cnst, size_t k) {
  size_t i, j, u, v;
  ...
  for (i = 0; i < k; i++) {
    ...
    for (j = 0; j < cnst->nz; j++) {
      ...
      polxvec_collaps_add_extension(&(*phi)[v + cnst->off[j]], alpha,
                                    cnst->phi[j], cnst->len[j],
...
int principle_prove(statement *ost, witness *owt, proof *pi,
                    const prncplstmnt *ist, const witness *iwt, int tail) {
  int ret;
  size_t i;
  constraint cnst[1] = {};
  void *buf = NULL;
...
int principle_verify(const prncplstmnt *st, const witness *wt) {
  ...
  if (wt->r != st->r) {
    fprintf(stderr,
            "ERROR in principle_verify(): Mismatch in witness multiplicity\n");
    return 1;
  }
  for (i = 0; i < wt->r; i++)
    if (wt->n[i] != st->n[i]) {
      fprintf(stderr,
              "ERROR in principle_verify(): Mismatch in length of witness "
              "vector %zu\n",
              i);
      return 2;
    }
  ...
  if (normsq > st->betasq) {
    fprintf(stderr,
            "ERROR in principle_verify(): Total witness vector norm too big\n");
    return 3;
  }
  ...
  *sx = NULL;
  for (i = 0; i < st->k; i++) {
    ...
    if (ret) {
      fprintf(stderr,
              "ERROR in principle_verify(): Sparse dot-product constraint %zu "
              "does not hold\n",
              i);
      ret = 10 + i;
      goto end;
