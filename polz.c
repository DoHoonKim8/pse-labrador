...
void polzvec_fromint64vec(polz *r, size_t len, size_t deg,
  ...
  for (i = 0; i < len; i++)
    for (j = 0; j < deg; j++)
      for (k = 0; k < N; k++)
        polz_setcoeff_fromint64(&r[i * deg + j], v[i * deg * N + k * deg + j],
                                k);
}
...
void polzvec_uniform(polz *r, size_t len, const uint8_t seed[16],
  ...
  size_t i, j, k;
  uint8_t *buf;
  const size_t chunk = (4096 / AES128CTR_BLOCKBYTES + QBYTES - 1) / QBYTES *
                       AES128CTR_BLOCKBYTES / N;
  ...
  while (len) {
    k = 0;
    nblocks =
        (len * N * QBYTES + AES128CTR_BLOCKBYTES - 1) / AES128CTR_BLOCKBYTES;
    __attribute__((aligned(64))) uint8_t buf2[nblocks * AES128CTR_BLOCKBYTES];
...
void polz_bitunpack(polz *r,
                    const uint8_t buf[(N / 32 * LOGQ + 15) / 16 * 64]) {
  int i, j, k, bits;
  __m512i f, g, h, mask;
...
void polzvec_almostuniform(polz *r, size_t len, const uint8_t seed[16],
...
#if LOGQ % 4 == 0
  chunk = (4 * 4096 / AES128CTR_BLOCKBYTES + LOGQ - 1) / LOGQ * 2 *
          AES128CTR_BLOCKBYTES / N;
#elif
  chunk = (4096 / AES128CTR_BLOCKBYTES + LOGQ - 1) / LOGQ * 8 *
          AES128CTR_BLOCKBYTES / N;
#endif
  ...
  if (len) {
    nblocks =
        (len * N * LOGQ / 8 + AES128CTR_BLOCKBYTES - 1) / AES128CTR_BLOCKBYTES;
    __attribute__((aligned(64))) uint8_t buf2[nblocks * AES128CTR_BLOCKBYTES];
    aes128ctr_squeezeblocks(buf2, nblocks, &aesctx);
    for (i = 0; i < len; i++)
...
static void polzvec_topolyvec_montgomery(poly *r, const polz *a, size_t len,
                                         size_t stride, const pdata *prime) {
  size_t i;
 ...
 * Explicit CRT mod q: \sum_i alpha_i(P/p_i mod q) - round(\sum_i alpha_i/p_i)(P
 * mod q)
 */
void polz_frompolx(polz *r, const polx *a) {
  ...
  for (i = 0; i < K; i++) {
    ...
    poly_quot_add(&k, &b.vec[i],
                  &primes[i]); // TODO: Overflow possible for K > 8
  }
...
static void frompolxvec(polz *r, const polx *a, size_t len) {
  ...
  for (i = 0; i < K; i++) {
    polyvec_scale(&b->vec[i], &b->vec[i], len, K, primes[i].t,
                  &primes[i]); // alpha_i
    polyvec_caddp(&b->vec[i], len, K,
                  &primes[i]); // needed for higher precision
    polyvec_quot_add(k, &b->vec[i], len, K,
                     &primes[i]); // TODO: Overflow possible for K > 8
  }
