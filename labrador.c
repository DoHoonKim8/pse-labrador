...
void init_comkey(size_t len) {
  ...
  if (comkey_len)
    polxvec_copy(buf, comkey, comkey_len);
  free(comkey);
  comkey = buf;
...
int init_proof(proof *pi, const witness *wt, int quadratic, int tail) {
  ...
  for (k = 15; k > 0; k--) {
    ...
    decompose =
        !tail &&
        !sis_secure(13, 6 * T * SLACK *
                            sqrt(2 * (TAU1 + 4 * TAU2) * varz * nn * N));
    ...
    for (cpp->kappa = 1; cpp->kappa <= 32; cpp->kappa++) {
      pi->normsq = (ldexp(1, 2 * cpp->b) / 12 * (cpp->f - 1) +
                    varz / ldexp(1, 2 * cpp->b * (cpp->f - 1))) *
                   nn;
      if (!tail) {
        pi->normsq += (ldexp(1, 2 * cpp->bu) * (cpp->fu - 1) +
                       ldexp(1, 2 * (LOGQ - (cpp->fu - 1) * cpp->bu))) /
                      12 * (rr * cpp->kappa + (rr * rr + rr) / 2);
      }
      if (!tail && quadratic)
        pi->normsq += (ldexp(1, 2 * cpp->bg) / 12 * (cpp->fg - 1) +
                       varg / ldexp(1, 2 * (cpp->fg - 1) * cpp->bg)) *
                      (rr * rr + rr) / 2;
      pi->normsq *= N;
      if (sis_secure(cpp->kappa, 6 * T * SLACK *
                                     ldexp(1, (cpp->f - 1) * cpp->b) *
                                     sqrt(pi->normsq)))
        break;
    }
    ...
    if (!tail) {
      ...
      if (quadratic)
        cpp->u1len += (rr * rr + rr) / 2;
      cpp->u2len = 2 * rr - 1;
      if (cpp->kappa <= 32 && (cpp->u1len + cpp->u2len) * LOGQ <=
                                  1.1 * nn * (log2(varz) / 2 + 2.05))
        break;
    }
  }
  ...
  if (cpp->kappa > 32) {
    fprintf(stderr,
            "ERROR in init_proof(): Cannot make inner commitments secure!\n");
    ret = 1;
    goto err;
  } else if (cpp->kappa1 > 32) {
    fprintf(stderr,
            "ERROR in init_proof(): Cannot make outer commitments secure!\n");
    ret = 2;
    goto err;
  }
  return 0;
...
void init_constraint_raw(constraint *cnst, size_t r, size_t n, size_t deg,
                         int quadratic) {
  size_t len;
  void *buf;
...
void init_statement(statement *st, const proof *pi, const uint8_t h[16]) {
  ...
  buf = _aligned_alloc(64, (st->cpp->u1len + st->cpp->u2len + st->r) *
                               sizeof(polx));
  st->u1 = (polx *)buf;
...
int set_witness_vector_raw(witness *wt, size_t i, size_t n, size_t deg,
                           const int64_t s[n * deg * N]) {
  if (i >= wt->r) {
    fprintf(stderr,
            "ERROR in set_witness_vector_raw(): Witness vector %zu does not "
            "exist\n",
            i);
    return 1;
  }
  if (n * deg != wt->n[i]) {
    fprintf(stderr, "ERROR in set_witness_vector_raw(): Mismatch of witness "
                    "vector length\n");
    return 2;
  }
...
void print_statement_pp(const statement *st) {
  ...
  printf("  Rank of outer commitment openings (inner coms, garbage): %zu\n",
         st->m);
  printf("  Number of non-zero coefficients in matrix a for quadratic "
         "constraint term: %zu\n",
         st->cnst->a->len);
...
double print_witness_pp(const witness *wt) {
  ...
  s = 0;
  for (i = 0; i < wt->r; i++)
    s += (log2((double)wt->normsq[i] / (N * wt->n[i])) / 2 + 2.05) * N *
         wt->n[i]; // FIXME: not necessarily gaussian
  s /= 8192;
...
size_t qugarbage_raw(polx *u, poly *g, size_t r, size_t n, const polx s[r][n],
  ...
  if (!tail) {
    ...
    off += polxvec_mul_extension(gx, &comkey[off], gx, cpp->fg * len,
                                 cpp->kappa1, 1);
...
void commit(statement *ost, witness *owt, proof *pi, polx sx[ost->r][ost->n],
            const witness *iwt) {
  ...
  size_t i, j, k, l;
  __attribute__((aligned(16))) uint8_t hashbuf[16 + cpp->u1len * N * QBYTES];
  ...
  for (i = 0; i < r; i++) {
    ...
    if (pi->nu[i]) {
      ...
      l = commit_raw(ost->u1, &v[t + j * cpp->fu * cpp->kappa], pi->nu[i],
                     ost->n, &sx[j], l, cpp, pi->tail);
      j += pi->nu[i];
      k = 0;
    }
  }
...
void reduce_commit(statement *ost, const proof *pi) {
  __attribute__((
      aligned(16))) uint8_t hashbuf[16 + pi->cpp->u1len * N * QBYTES];
...
int project(statement *ost, proof *pi, uint8_t jlmat[][ost->n][256 * N / 8],
            const witness *iwt) {
  const size_t r = iwt->r;
  const size_t *n = iwt->n;
  ...
  __attribute__((aligned(16))) uint8_t hashbuf[16 + 1024];
  ...
  do {
    ...
    j = k = 0;
    for (i = 0; i < r; i++) {
      aes128ctr_squeezeblocks(
          jlmat[j][k], n[i] * 256 * N / 8 / AES128CTR_BLOCKBYTES, &aesctx);
...
int reduce_project(statement *ost, uint8_t jlmat[][ost->n][256 * N / 8],
  ...
  if (jlproj_normsq(pi->p) > 256 * MIN(JLMAXNORMSQ, betasq)) {
    fprintf(
        stderr,
        "ERROR in reduce_project(): Witness projection longer than bound\n");
    return 1;
  }
  ...
  for (i = 0; i < r; i++) {
    aes128ctr_squeezeblocks(
        jlmat[j][k], pi->n[i] * 256 * N / 8 / AES128CTR_BLOCKBYTES, &aesctx);
    k += pi->n[i];
    if (pi->nu[i]) {
      memset(jlmat[j][k], 0, (pi->nu[i] * ost->n - k) * 256 * N / 8);
      j += pi->nu[i];
...
void collaps_jlproj_raw(constraint *cnst, size_t r, size_t n, uint8_t h[16],
                        const int32_t p[256],
                        const uint8_t jlmat[r][n][256 * N / 8]) {
  __attribute__((aligned(
      32))) uint8_t hashbuf[32 + QBYTES * 256 +
                            24]; // additional 24 bytes because of vector loads
  int64_t x;
...
void collaps_jlproj(constraint *cnst, statement *st, const proof *pi,
                    const uint8_t jlmat[st->r][st->n][256 * N / 8]) {
  collaps_jlproj_raw(cnst, st->r, st->n, st->h, pi->p, jlmat);
}

void lift_aggregate_zqcnst(statement *ost, proof *pi, size_t i,
                           constraint *cnst, const polx sx[ost->r][ost->n]) {
  __attribute__((aligned(16))) uint8_t hashbuf[16 + N * QBYTES];
  polx alpha[1];
...
void reduce_lift_aggregate_zqcnst(statement *ost, const proof *pi, size_t i,
...
static void aggregate(statement *ost, const proof *pi, const statement *ist) {
  ...
  __attribute__((aligned(16))) uint8_t hashbuf[32];
  ...
  j = 0;
  for (i = 0; i < r; i++)
    j += polxvec_collaps_add_extension(&phiv[t + i * l], alpha, &comkey[j], l,
                                       cpp->kappa1, 1);
  polxvec_collaps_add_extension(&phiv[g], alpha, &comkey[j], h - g, cpp->kappa1,
                                1);
  ...
  for (i = 0; i < cpp->f; i++) {
    if (i)
      polxvec_scale(tmp, tmp, n, (int64_t)1 << cpp->b);
  ...
  for (i = 0; i < r; i++) {
    ...
    for (j = 0; j < cpp->fu; j++) {
      if (j)
        polxvec_scale(tmp, tmp, cpp->kappa, (int64_t)1 << cpp->bu);
      polxvec_add(&phiv[t + i * l + j * cpp->kappa],
                  &phiv[t + i * l + j * cpp->kappa], tmp, cpp->kappa);
    }
  }
  ...
  for (i = 0; i < cpp->fg; i++) {
    if (i)
      polxvec_scale(&tmp[k], &tmp[k], k, (int64_t)1 << cpp->bg);
    polxvec_add(&phiv[g + i * k], &phiv[g + i * k], &tmp[k], k);
  }
  for (i = 0; i < cpp->fu; i++) {
    if (i)
      polxvec_scale(&tmp[0], &tmp[0], k, (int64_t)1 << cpp->bu);
  ...
  if (ist->cnst->a->len) {
    ...
    ost->cnst->a->coeffs =
        realloc(ost->cnst->a->coeffs, ost->cnst->a->len * sizeof(polx));
  }

  free(tmp);
}

static void amortize_tail(statement *ost, witness *owt, proof *pi,
                          polx sx[ost->r][ost->n]) {
  ...
  size_t i;
  __attribute__((aligned(16))) uint8_t hashbuf[16 + 2 * N * QBYTES];
...
void amortize(statement *ost, witness *owt, proof *pi,
              polx sx[ost->r][ost->n]) {
  const size_t r = ost->r;
  const size_t n = ost->n;
  ...
  /* amortization */
  __attribute__((aligned(16))) uint8_t hashbuf[16 + cpp->kappa1 * N * QBYTES];
  memcpy(hashbuf, ost->h, 16);
...
int reduce_amortize(statement *ost, const proof *pi) {
  ...
  if (!sis_secure(cpp->kappa, 6 * T * SLACK * ldexp(1, (cpp->f - 1) * cpp->b) *
                                  sqrt(ost->betasq))) {
    fprintf(stderr,
            "ERROR in reduce_amortize(): Inner commitments not secure\n");
    return 1;
  }
  if (!pi->tail && !sis_secure(cpp->kappa1, 2 * SLACK * sqrt(ost->betasq))) {
    fprintf(stderr,
            "ERROR in reduce_amortize(): Outer commitments not secure\n");
    return 2;
  }
  ...
  __attribute__((aligned(16))) uint8_t hashbuf[16 + cpp->u2len * N * QBYTES];
  memcpy(hashbuf, ost->h, 16);
...
int prove(statement *ost, witness *owt, proof *pi, const statement *ist,
          const witness *iwt, int tail) {
  int ret;
  size_t i;
  constraint cnst[1] = {};
  void *buf = NULL;
...

  {
    ...
    uint8_t(*jlmat)[ost->n][256 * N / 8] =
        (uint8_t(*)[ost->n][256 * N / 8]) sx[ost->r];
...
int verify(const statement *st, const witness *wt) {
  ...
  if (normsq > st->betasq) {
    fprintf(stderr,
            "ERROR in verify(): Total witness norm bigger than bound\n");
    ret = 1;
    goto end;
  }
  ...
  if (!st->tail) {
    ...
    for (i = 0; i < r; i++) {
      ...
      j += polxvec_mul_extension(tmp1, &comkey[j], &v[t + i * l], l,
                                 cpp->kappa1, 1);
    ...
    if (!polxvec_iszero(tmp0, cpp->kappa1)) {
      fprintf(stderr,
              "ERROR in verify(): First outer commitment opening wrong\n");
      ret = 2;
      goto end;
    }
    ...
    if (!polxvec_iszero(tmp0, cpp->kappa1)) {
      fprintf(stderr,
              "ERROR in verify(): Second outer commitment opening wrong\n");
      ret = 3;
      goto end;
    }
    ...
    for (i = 0; i < r; i++)
      for (j = 1; j < cpp->fu; j++)
        polxvec_scale_add(&v[t + i * l], &v[t + i * l + j * cpp->kappa],
                          cpp->kappa, (int64_t)1 << j * cpp->bu);
    for (i = 1; i < cpp->fg; i++)
      polxvec_scale_add(&v[g], &v[g + i * (r * r + r) / 2], (r * r + r) / 2,
                        (int64_t)1 << i * cpp->bg);
    for (i = 1; i < cpp->fu; i++)
      polxvec_scale_add(&v[h], &v[h + i * (r * r + r) / 2], (r * r + r) / 2,
                        (int64_t)1 << i * cpp->bu);
  } else {
  ...
  if (!polxvec_iszero(tmp0, cpp->kappa)) {
    fprintf(stderr,
            "ERROR in verify(): Amortized (inner commitment) opening wrong\n");
    ret = 4;
    goto end;
  }
  ...
  if (!polxvec_iszero(tmp0, 1)) {
    fprintf(
        stderr,
        "ERROR in verify(): Aggregated dot-product constraint doesn't hold\n");
    ret = 5;
    goto end;
  }
  ...
  if (st->cnst->a->len) {
    ...
    for (i = 0; i < r; i++) {
      ...
      if (i)
        polx_mul_add(&v[g], &st->c[i], tmp0);
      else
        polx_mul(&v[g], &st->c[i], tmp0);
      k += r - i;
    }
    ...
    if (!polx_iszero(tmp0)) {
      fprintf(stderr,
              "ERROR in verify(): Quadratic garbage polynomials wrong\n");
      ret = 6;
      goto end;
    }
  }
  ...
  if (!st->tail) {
    ...
    for (i = 0; i < r; i++) {
      ...
      if (i)
        polx_mul_add(&v[h], &st->c[i], tmp0);
      else
        polx_mul(&v[h], &st->c[i], tmp0);
      k += r - i;
    }
  } else {
