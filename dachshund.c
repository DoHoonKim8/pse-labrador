...
int init_smplstmnt_raw(smplstmnt *st, size_t r, const size_t n[r],
                       const uint64_t betasq[r], size_t k) {
  size_t i;
  void *buf;
...
int set_smplstmnt_lincnst_raw(smplstmnt *st, size_t i, size_t nz,
...
static void init_commitment(commitment *com, const comparams *cpp,
                            const witness *wt) {
  size_t i;
  void *buf;
...
static int expand_witness(witness *ewt, const smplstmnt *ist,
                          const witness *iwt) {
  int ret;
  size_t i, k;
  ...
  for (i = 0; i < r + 1; i++) {
    if ((i == r || !ist->betasq[i]) &&
        normsq + sqrt(normsq * ewt->n[i] * N) > ldexp(1, LOGQ)) {
      fprintf(stderr,
              "ERROR: Total witness norm too big to support binary proof for "
              "witness vector %zu\n",
              i);
      ret = 1;
      goto err;
    ...
    if ((i < r && ist->betasq[i]) &&
        normsq > ((uint64_t)1 << (LOGQ - 1)) - QOFF) {
      fprintf(stderr,
              "ERROR: Total witness norm too big to support exact l2-norm "
              "proof for witness vector %zu\n",
              i);
      ret = 2;
      goto err;
    }
...
static void simple_commit(statement *ost, witness *owt, proof *pi,
                          commitment *com, polx sx[ost->r][ost->n],
  ...
  __attribute__((aligned(
      16))) uint8_t hashbuf[16 + MAX(cpp->u1len, com->kappa1) * N * QBYTES];
  poly *v = owt->s[cpp->f];
  polx gdgt[1];
  polz tmp[1];
...
static void reduce_simple_commit(statement *ost, const proof *pi,
                                 const commitment *com) {
  __attribute__((aligned(
      16))) uint8_t hashbuf[16 + MAX(pi->cpp->u1len, com->kappa1) * N * QBYTES];
...
static void simple_collaps(constraint *cnst, statement *ost, const proof *pi,
                           const commitment *com, const smplstmnt *ist) {
...
static void simple_aggregate(statement *ost, const proof *pi,
                             const commitment *com, const smplstmnt *ist) {
  const size_t r = ist->r + 1;
  const size_t s0 = 0;
  const size_t s1 = s0 + pi->nu[r - 1];
...
int simple_prove(statement *ost, witness *owt, proof *pi, commitment *com,
...

  {
    ...
    buf = _aligned_alloc(64, ost->r * ost->n * sizeof(polx) +
                                 s1 * ost->n * 256 * N / 8);
...
int simple_reduce(statement *ost, const proof *pi, const commitment *com,
                  const smplstmnt *ist) {
  size_t i;
  int ret;
  uint64_t betasq = 0;
...
int simple_verify(const smplstmnt *st, const witness *wt) {
  ...
  if (wt->r != st->r) {
    fprintf(stderr,
            "ERROR in simple_verify(): Mismatch in witness multiplicity\n");
    return 1;
  }
  for (i = 0; i < wt->r; i++) {
    if (wt->n[i] != st->n[i]) {
      fprintf(stderr,
              "ERROR in simple_verify(): Mismatch in length of witness vector "
              "%zu\n",
              i);
      return 2;
    }
    ...
    if (st->betasq[i] && wt->normsq[i] > st->betasq[i]) {
      fprintf(
          stderr,
          "ERROR in simple_verify(): l2-Norm of vector %zu larger than bound\n",
          i);
      return 4;
    }
  }
  ...
  *sx = NULL;
  for (i = 0; i < st->k; i++) {
    ...
    if (ret) {
      fprintf(stderr,
              "ERROR in simple_verify(): Sparse dot-product constraint %zu "
              "does not hold\n",
              i);
      ret = 10 + i;
      goto end;
