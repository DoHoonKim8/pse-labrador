...
/* allocate statement for r witness vectors of ranks n[r] and l2-norm bounds
 * betasq[r] (0 for 0/1), consisting of k linear constraints (rows of constraint
 * matrix) */
#define init_smplstmnt_raw NAMESPACE(init_smplstmnt_raw)
__attribute__((visibility("default"))) int
init_smplstmnt_raw(smplstmnt *st, size_t r, const size_t n[r],
                   const uint64_t betasq[r], size_t k);
/* set j-th linear constraint (j <= k) where the nz sub-vectors of indices
 * idx[nz] (0 <= idx[nz] < r) are non-zero; phi is the concatenation of the
 * non-zero sub vectors (of length \sum_{i=0}^{nz-1} n[i]) */
...
__attribute__((visibility("default"))) void free_commitment(commitment *com);
...
__attribute__((visibility("default"))) int simple_verify(const smplstmnt *st,
                                                         const witness *wt);
